package CliniqueManagement;
	...
	public class CliniqueImplemented implements CliniqInterface
	{
		...
		public void readJson() {
			...
			if (fileDoctor.exists() && fileDoctor.length() > 0) {
				...
				doctors.addAll(modelDoctor.getDoctorList());
			...
			if (filePatient.exists() && filePatient.length() > 0) {
				...
				patients.addAll(modelPatient.getPatientList());
			...
			if (fileAppointment.exists() && fileAppointment.length() > 0) {
				...
				appointments.addAll(modelAppointment.getAppointmentList());
		...
		public void savingAppointment(Appointment appointment) {
			...
			modelAppointment.setAppointmentList(appointments);
			try {
		...
		public Appointment getAppointment(int index, int patientId) {
			...
			modelDoctor.setDoctorList(doctors);
		...
		public void save() {
			...
			// saving patient
			if (patients.size() > 0) {
				modelPatient.setPatientList(patients);
				try {
